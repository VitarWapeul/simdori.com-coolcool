 ...
 * @since       Android 5.1
 */
public class Set_IotDeleteAsyncTask extends AsyncTask<String, Void, Integer> {
    // API
     ...
     * @param id			: 사용자 입력 아이디
     */
    public Set_IotDeleteAsyncTask(Context context, String id) {
        this.context = context;
        this.id = id;

        // 재시도 횟수
        pref = new SharedPrefUtil(context);
        retry_request = pref.getValue(SharedPrefUtil.RETRY_REQUEST+"deleteDevice", 0);

        // 연결 정보
        json = new JsonConnect(context, AppConst.deleteDevice_host);

        // 현재언어 가져오기
        locale = context.getResources().getConfiguration().locale;
        language =  locale.getLanguage();
        if(AppConst.DEBUG_ALL) Log.d(AppConst.TAG, "deleteDevice 현재언어는 " + language);
    }
    ...
    /**
     * <Request>
     * uri			: http://simdori.com:3000/deleteDevice;
     * id		    : 사용자 입력 아이디
     * lang			: 현재 언어
     * <Response>
     * resultCode	: 1(성공)
    ...
    protected Integer doInBackground(String... arg0) {
        ...
        if(retry_request > AppConst.Network_Retry_Baseline) {
            ...
            try {
                ...
                if(AppConst.DEBUG_ALL) Log.i(AppConst.TAG, "deleteDevice Data_Request = " + job.toString());

                // Json Data 처리
                response = json.Connect(job);
                if(AppConst.DEBUG_ALL) Log.i(AppConst.TAG, "deleteDevice Data_Response = " + response);
