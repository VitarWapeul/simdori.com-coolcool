 ...
 * @since       Android 5.1
 */
public class Set_UserSensorAsynctask extends AsyncTask<String, Void, Integer> {
    // API
    ...
    // Values
    private String id, language, no = null;
     ...
     * @param id			: 사용자 입력 아이디
     */
    public Set_UserSensorAsynctask(Context context, String id, String no) {
        ...
        retry_request = pref.getValue(SharedPrefUtil.RETRY_REQUEST+"selectBPMDetail", 0);

        // 연결 정보
        json = new JsonConnect(context, AppConst.Select_BPMDetail_host);
    ...
    protected Integer doInBackground(String... arg0) {
        ...
        if(retry_request > AppConst.Network_Retry_Baseline) {
            ...
            try {
                ...
                job.put("no", no);
                if(AppConst.DEBUG_ALL) Log.i(AppConst.TAG, "SelectBPMDetail Data_Request = " + job.toString());

                // Json Data 처리
                response = json.Connect(job);
                if(AppConst.DEBUG_ALL) Log.i(AppConst.TAG, "SelectBPMDetail Data_Response = " + response);
