 ...
 * @since       Android 5.1
 */
public class Set_IotAsyncTask extends AsyncTask<String, Void, Integer> {
    // API
    ...
    // Values
    private String id, language, device_mac = null;
     ...
     * @param id			: 사용자 입력 아이디
     * @param device_mac	: 등록할 장치 mac 주소
     */
    public Set_IotAsyncTask(Context context, String id, String device_mac) {
        ...
        retry_request = pref.getValue(SharedPrefUtil.RETRY_REQUEST+"registerDevice", 0);

        // 연결 정보
        json = new JsonConnect(context, AppConst.registerDevice_host);

        // 현재언어 가져오기
        locale = context.getResources().getConfiguration().locale;
        language =  locale.getLanguage();
        if(AppConst.DEBUG_ALL) Log.d(AppConst.TAG, "registerDevice 현재언어는 " + language);
    }
    ...
    /**
     * <Request>
     * uri			: http://simdori.com:3000/registerDevice;
     * id		    : 사용자 입력 아이디
     * lang			: 현재 언어
     * <Response>
     * resultCode	: 1(성공)
    ...
    protected Integer doInBackground(String... arg0) {
        ...
        if(retry_request > AppConst.Network_Retry_Baseline) {
            ...
            try {
                ...
                if(AppConst.DEBUG_ALL) Log.i(AppConst.TAG, "registerDevice Data_Request = " + job.toString());

                // Json Data 처리
                response = json.Connect(job);
                if(AppConst.DEBUG_ALL) Log.i(AppConst.TAG, "registerDevice Data_Response = " + response);
